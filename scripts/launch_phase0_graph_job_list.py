...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Launch selected graph Phase-0 jobs across GPUs.")
    p.add_argument("--jobs-csv", type=Path, required=True, help="CSV with task,condition,seed columns")
    p.add_argument("--artifact-root", type=Path, default=Path("artifacts/phase0_graph"))
...
def main() -> None:
    ...
    slots = [gpu for gpu in gpus for _ in range(args.max_parallel_per_gpu)] or ["cpu"]
    ...
    active: list[tuple[subprocess.Popen, str, tuple[str, str, int], object]] = []
    print(f"Using slots: {slots}", flush=True)
    print(f"Queued jobs: {jobs.qsize()}", flush=True)

    while not jobs.empty() or active:
        alive = []
        for proc, gpu, meta, handle in active:
            if proc.poll() is None:
                alive.append((proc, gpu, meta, handle))
            else:
                handle.close()
        active = alive
        while not jobs.empty() and len(active) < len(slots):
            used = {gpu for _, gpu, _, _ in active}
            ...
            cmd = [
                sys.executable,
                "scripts/phase0_run_graph_one.py",
                "--task",
                task,
                "--condition",
                condition,
            ...
            active.append((proc, gpu, (task, condition, seed), handle))
        if active:
            time.sleep(15)
